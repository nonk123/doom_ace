...
void* hack_malloc(size_t size)
{
    ...
    if (result == NULL) {
        engine_error("RIBBIT", "Ribbit VM wanted to malloc(), but it failed");
    }
...
{
    if (ptr == NULL) {
        engine_error("RIBBIT", "Ribbit VM tried to free() a null pointer");
    }
...
{
    if (file != HACK_STDOUT) {
        engine_error("RIBBIT", "`fflush` called outside of the ribbit module; this should never happen!");
    }
...
// HACK: non-destructively mangle the original VM code to use whatever shit the ACE engine provides.
...
void init_ribbit()
{
    ...
    // Initial run. Afterwards, it is run every tick. (except it isn't yet)
    run_ribbit();
}

#define ADVANCE_PC pc = TAG(pc)
...
void run_ribbit()
{
    ...
    while (!halt) {
        ...
        switch (instr) {
        default: { // error
            engine_error("RIBBIT", "Error in Lisp code: %d", instr);
            break;
        }
        case INSTR_HALT: {
            halt = true;
...
#define code CAR(proc)
            ...
            if (IS_NUM(code)) {
                ...
                if ((!vari && nparams != nargs) || (vari && nparams > nargs)) {
                    engine_error("RIBBIT", "Unexpected number of arguments nargs: %ld nparams: %ld vari: %ld\n", nargs, nparams, vari);
                }
                // )@@
