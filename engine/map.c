...
static const map_attr_t map_attr[] = {
    ...
    {.name = "monsterfallingdamage",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
     .flag = MAP_FLAG_MONSTER_FALL_DMG_KILL},
    {.name = "propermonsterfallingdamage",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
    ...
    {.name = "totalinfighting",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
     .flag = MAP_FLAG_TOTAL_INFIGHTING},
    //	{.name = "checkswitchrange", .type = IT_FLAG, .offset =
    // offsetof(map_level_t, flags), .flag = MAP_FLAG_CHECK_SWITCH_RANGE},
    {.name = "resetinventory",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
     .flag = MAP_FLAG_RESET_INVENTORY},
    //	{.name = "forgetstate", .type = IT_FLAG, .offset = offsetof(map_level_t,
    // flags), .flag = MAP_FLAG_FORGET_STATE},
    {.name = "spawnwithweaponraised",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
     .flag = MAP_FLAG_SPAWN_WITH_WEAPON_RAISED},
    {.name = "strictmonsteractivation",
     .type = IT_FLAG,
     .offset = offsetof(map_level_t, flags),
     .flag = MAP_FLAG_NO_MONSTER_ACTIVATION},
    // terminator
    {.name = NULL}};
// cluster attributes
...
static void spawn_map_thing(map_thinghex_t *mt, mapthing_t *ot)
{
	...
	// special types
	if (mt && (mt->type == 1411 || // sound sequence
	...
	while (1)
	{
		...
		switch (attr->type)
		{
			...
			case IT_MAP:
			{
				...
				else if (!strcmp("endgame3", kw))
				{
					next = MAP_END_BUNNY_SCROLL;
				}
				else if (!strcmp("endtitle", kw))
				{
	...
	while (1)
	{
		...
		if (!strcmp(kw, kwl->lock))
		{
			...
			while (1)
			{
				...
				else if (kwl->msg_data && (!strcasecmp(kw, kwl->msg_data) || !strcasecmp(kw, kwl->rmt_data)))
				{
					uint16_t *ret;
					uint32_t addr;
					...
					ret = lockdefs_add_text(
					    bptr, buffer + 256, (uint8_t *)addr + doom_data_segment, code
					);
					if (!ret)
						goto error_overflow;
				...
				else if (!strcasecmp(kw, kwl->mapcolor))
				{
					uint32_t r, g, b;
...
__attribute((regparm(2), no_caller_saved_registers)) static void do_new_game()
{
	gameaction = ga_nothing;
...
__attribute((regparm(2), no_caller_saved_registers)) static void set_world_done()
{
	...
	else if (old_cl && old_cl->flags & CLST_FLAG_HUB)
		save_hub_level();
...
__attribute((regparm(2), no_caller_saved_registers)) static void do_completed()
{
	uint16_t next;
...
__attribute((regparm(2), no_caller_saved_registers)) static void do_world_done()
{
	map_cluster_t *cluster;
...
__attribute((regparm(2), no_caller_saved_registers)) static void draw_finished()
{
	int32_t y = WI_TITLEY;
	patch_t *patch;
...
__attribute((regparm(2), no_caller_saved_registers)) static void draw_entering()
{
	int32_t y = WI_TITLEY;
	patch_t *patch;
...
uint32_t
P_BlockLinesIterator(int32_t x, int32_t y, line_func_t func)
{
	uint16_t *list;
	uint32_t idx;
	...
	// apparently, first entry is always zero and does not represent line
	// IDX
	if (!*list)
		list++;
...
static const hook_t hooks[] __attribute__((used, section(".hooks"), aligned(4))) = {
    ...
    // replace pointers to 'PTR_SlideTraverse'
    // in 'P_SlideMove'
