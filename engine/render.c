		...
		if ((tcol[dc_x] & 0x8000) == masked_col_step)
		{
			...
			if (texnum >= 0)
			{
				...
				else
				{
					...
					// BEWARE: this does not work well with
					// positive 'rowoffset'
...
static __attribute((regparm(2), no_caller_saved_registers)) void R_DrawVisSprite(vissprite_t *vis)
{
	...
	else
	{
		...
		if (vis->mo->translation)
		{
			...
			else if (colfunc == R_DrawColumnTint0)
				colfunc = R_DrawTranslatedColumnTint0;
			else if (colfunc == R_DrawColumnTint1)
...
static inline void draw_masked_range()
{
	...
	if (e3d_dbg)
	{
		e3d_dbg = 0;
		doom_sprintf(
		    exitmsg, "sp: %u pv: %u pt: %u", e3d_dbg_splitcount, e3d_dbg_planecount, e3d_count_planes()
		);
...
static
    ...
    void
    r_add_line(seg_t *line, int32_t x2, int32_t x1)
{
...
static __attribute((regparm(2), no_caller_saved_registers)) void R_Subsector(uint32_t num)
{
	...
	while (pl)
	{
		...
		{
			// out of sight; but must add height for light effects
			// and sides
			if (pl->light)
				light = *pl->light;
			pl = pl->next;
...
static void generate_sector_light(uint8_t *dest, uint16_t color, uint16_t fade)
{
	...
	if (!sector_light_warning)
	{
		...
		messageString = "Generating colored light.\nThis will take a "
				"while ...\n\nInclude generated tables in WAD "
				"file\nto speed this up!";
		vesa_copy();
		M_Drawer();
		vesa_update();
...
void render_preinit(uint8_t *palette)
{
	...
	for (uint32_t i = 0; i < 256; i++, pal++)
	{
		...
		// This calculation when used for sector light does not match
		// ZDoom but it looks much better.
		pal->l = pal->r;
		if (pal->l < pal->g)
			pal->l = pal->g;
		if (pal->l < pal->b)
...
static __attribute((regparm(2), no_caller_saved_registers)) void hook_RenderPlayerView(player_t *pl)
{
	...
	if (pl->text.text)
	{
		...
		font_center_text(
		    SCREENWIDTH / 2, SCREENHEIGHT / 2, pl->text.text, font_load(pl->text.font), pl->text.lines
		);
	}
