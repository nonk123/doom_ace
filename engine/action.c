...
void missile_stuff(
    mobj_t *mo, mobj_t *source, mobj_t *target, fixed_t speed, angle_t angle, angle_t pitch, fixed_t slope
)
{
...
uint8_t *action_parser(uint8_t *name)
{
	...
	if (parse_arg_idx < act->minarg)
		engine_error(
		    "DECORATE", "Missing arg[%d] for action '%s' in '%s'!", parse_arg_idx, name, parse_actor_name
		);
...
static const dec_action_t mobj_action[] = {
    ...
    {"a_vileattack",
     ...
     {STRARG(0, STR_SOUND), STRARG(5, STR_DAMAGE_TYPE)}}, // "sound", initialdamage, blastdamage,
                                                          ...
                                                          // flags player attack
    {"a_fireprojectile", A_FireProjectile, 7, 1, {STRARG(0, STR_MOBJ_TYPE)}}, // "missiletype", angle, useammo,
                                                                              // spawnofs_xy, spawnheight, flags, pitch
    ...
    {"a_explode", A_Explode, 5, 1},                                                   // damage, distance, flags, alert,
                                                                                      // fulldamagedistance health
