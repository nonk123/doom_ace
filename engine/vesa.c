...
static __attribute((regparm(2), no_caller_saved_registers)) void swap_buffers()
{
...
static __attribute((regparm(2), no_caller_saved_registers)) void stop_vesa()
{
...
static uint32_t check_mode_info(uint16_t mode_id, uint32_t do_list)
{
	...
	if (dpmiregs.eax.x == 0x004F)
	{
		if (do_list)
			doom_printf(
			    "[VESA] mode 0x%04X: %ux%ux%u %u 0x%04X 0x%08X\n", mode_id, mode->width, mode->height,
			    mode->bpp, mode->image_pages, mode->attributes, (uint32_t)mode->framebuffer
			);
		...
		if (info->version >= 0x0200)
		{
			...
			while (1)
			{
				...
				if (last_mode == *mode_list)
				{
					...
					// sometimes 'video_modes' pointer is
					// broken
					last_mode = 0xFFFF;
					break;
				}
