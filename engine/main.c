...
static void cb_counter(lumpinfo_t* li) { loading->counter_value++; }
...
void zone_info()
{
	...
	for (block = mainzone->blocklist.next;; block = block->next)
	{
		...
		// block->tag, block->size, block->user);
...
static __attribute((regparm(2), no_caller_saved_registers)) void Z_Init()
{
	...
	if (total < check && !M_CheckParm("-lowzone"))
	{
		...
		engine_error("ZONE",
		             "Not enough memory!\nMinimum is %u %ciB but only "
		             "%u.%02u %ciB is available.\nYou can skip this "
		             "check using parameter '%s'.",
		             check, s0, total, dec, s1, "-lowzone");
	}
