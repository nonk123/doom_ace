...
const dec_inherit_t inheritance[NUM_EXTRA_TYPES] = {
    [ETYPE_NONE] = {.name = NULL, .def = &default_mobj},
    [ETYPE_POWERUP_BASE] =
	{
	    .attr[0] = attr_powerup,
...
static const state_t internal_states[] = {
    [STATE_UNKNOWN_ITEM - NUMSTATES] =
	{
	    ...
	    .frame = 0,
	    .tics = 0xFFFF,
	    .nextstate = 0, // this state is used for delayed deletion so 'next'
                            // has to be zero
	},
...
static uint32_t parse_attr(uint32_t type, void *dest)
{
	...
	switch (type)
	{
		...
		case DT_MOBJTYPE:
			...
			if (num.s32 < 0)
				engine_error(
				    "DECORATE", "Unable to find ammo type '%s' in '%s'!", kw, parse_actor_name
				);
		...
		case DT_DROPITEM:
			if (parse_mobj_info->extra_type == ETYPE_PLAYERPAWN)
				engine_error(
				    "DECORATE", "Attempt to add DropItem to player class '%s'!", parse_actor_name
				);
...
static uint32_t parse_wpn_slot()
{
	...
	while (1)
	{
		...
		if (tmp < 0)
			engine_error(
			    "DECORATE", "Unable to find '%s' in '%s' weapon slot %u!", kw, parse_actor_name, slot
			);
	...
	have_keyword:
		...
		while (1)
		{
			...
			if (!strcmp(kw, "offset"))
			{
				...
				flags |= FF_FULLBRIGHT;
			else if (!strcmp(kw, "fast"))
				flags |= FF_FAST;
			else if (!strcmp(kw, "nodelay"))
				flags |= FF_NODELAY;
			else if (!strcmp(kw, "canraise"))
				flags |= FF_CANRAISE;
			else
				break;
		}
		...
		while (1)
		{
			...
			if (kw[0] == '-' || kw[0] == '+')
			{
				...
				if (change_flag(kw, mobj_flags0, offsetof(mobjinfo_t, flags)) &&
					...
					engine_error(
					    "DECORATE", "Unknown flag '%s' in '%s'!", kw + 1, parse_actor_name
					);
			}
			else
			{
				...
				if (!attr->name)
					engine_error(
					    "DECORATE", "Unknown attribute '%s' in '%s'!", kw, parse_actor_name
					);
		...
		for (uint32_t i = first_state; i < num_states; i++)
		{
			...
			if ((st->next_extra & STATE_CHECK_MASK) == STATE_SET_ANIMATION)
			{
				...
				next = dec_resolve_animation(
				    info, st->next_extra & STATE_EXTRA_MASK, st->nextstate, num_states
				);
	...
	for (uint32_t idx = 0; idx < num_mobj_types; idx++)
	{
		...
		switch (info->extra_type)
		{
			...
			case ETYPE_POWERUP:
			{
				...
				ofni->eflags &= MFE_INVENTORY_HUBPOWER | MFE_INVENTORY_PERSISTENTPOWER |
