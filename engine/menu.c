...
static __attribute((regparm(2), no_caller_saved_registers)) void display_draw()
{
	...
	// wipe
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 2, wipe_name[extra_config.wipe_type]
	);

	// FPS
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 3, off_on[!!extra_config.show_fps]
	);
...
static __attribute((regparm(2), no_caller_saved_registers)) void mouse_draw()
{
	...
	for (uint32_t i = 0; i < NUM_MOUSE_CTRL; i++)
		font_menu_text(
		    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * (i + 2), control_btn_name(i)
		);
}
...
static __attribute((regparm(2), no_caller_saved_registers)) void player_draw()
{
	...
	// auto switch
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 1, off_on[!!extra_config.auto_switch]
	);

	// auto aim
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 2, off_on[!!extra_config.auto_aim]
	);

	// mouse look
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 3, off_on[extra_config.mouse_look]
	);

	// center weapon
	font_menu_text(
	    options_menu.x + 100, -options_menu.y + mod_config.menu_font_height * 4,
...
static __attribute((regparm(2), no_caller_saved_registers)) void menu_items_draw(menu_t *menu)
{
	...
	if (y < 0)
	{
		...
		if (menu->menuitems[menu_item_now].status == 2)
			V_DrawPatchDirect(
			    x + CURSORX_SMALL, y + menu_item_now * mod_config.menu_font_height, selector_special
			);
		else
			V_DrawPatchDirect(
			    x + CURSORX_SMALL, y + menu_item_now * mod_config.menu_font_height, selector_normal
			);
